...
class CarController:
  ...
  def update(self, CC, CS):
    ...
    e2e_long_chime = self.e2e_long_alert and self.e2e_long_status == 2 and not hud_control.leadVisible and \
                     ...
                     not CS.out.gasPressed and CS.out.standstill and (1 < (cur_time - self.e2e_long_status_timer) <= 2)
    ...
    if not self.CP.openpilotLongitudinalControl:
      ...
      if not CC.cruiseControl.resume and CS.out.cruiseState.enabled and not CS.out.gasPressed and not self.CP.pcmCruiseSpeed:
        ...
        if self.cruise_button is not None and (self.frame - self.last_button_frame) * DT_CTRL > 0.1:
